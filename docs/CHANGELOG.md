...
- 表头只作为显示文字，内部操作统一使用稳定且唯一的列 ID，原始“序号”列和重复表头不再引发冲突。
- 筛选、列选择、合并、替换和模板按列引用工作，同名列可以被独立选择与恢复。
- CSV、Excel 和多行 TAB 剪贴板原样保留重复表头，不再自动追加 `_2` / `_3`。
- 新增保守的特征评分表头识别，以及可撤销的“强制首行作表头”纠错操作。
- 配置结构升级为 `schemaVersion: 2`；旧配置仅备份并初始化空白新配置，不执行自动迁移。
- 核心回归扩展至 `29/29`，正式单文件 Release 继续执行更新器资源和关键 UI 自检。

## v1.1.2 - 2026-07-13

### 在线更新链验证版

- 使用已下沉到 `DesktopUpdateKit` 的共享发布清理与最终 EXE 自检流程构建。
- 用作 v1.1.1 到 v1.1.2 的真实在线下载、校验、UpdaterStub 替换、健康检查与回滚链验证。
- 不包含新的数据处理功能变更。
...
- 修复下载中、下载完成及启动更新失败状态下，状态文字、进度条、操作按钮和更新说明互相遮挡的问题。
- 更新窗口改用按 DPI 缩放的动态纵向布局，状态文字可自动换行，更新说明文本框填充其容器且不再默认选中文字。
- 修复普通 Release 构建残留被增量发布复用，导致正式 EXE 未内嵌 `UpdaterStub`、下载成功后无法启动安装的问题。
- 正式发布入口会先清理 Release 增量输出，并对最终单文件 EXE 执行更新器资源与关键 UI 布局自检；制包前会再次验证。
- 上述清理与最终 EXE 自检已下沉到共享 `DesktopUpdateKit`；宿主通过 `releaseVerificationArguments` 声明无副作用的自检开关。

## v1.1.0 - 2026-07-13

### 完整代码复审与结果正确性加固

- 分隔记录统一构造成等宽表格；数据行比表头更宽时自动补充表头，不再静默丢弃尾部单元格。
- 剪切板解析改用支持引号、转义引号和单元格内换行的统一解析器；单行剪切板内容不再同时充当表头和重复数据。
- 修复模板矩阵从剪切板导入时漏掉第一行的问题。
- 修复保留全部列但改变顺序时未按用户顺序重排的问题。
- 分组列与求和列冲突时在 UI 和 Core 双重阻断；行列合并、模板和替换失败时保持原数据与撤销栈不变。
- 筛选条件正确转义 `%`、`*`、`[` 等字符；筛选变化会清除旧行选择，避免后续操作命中错误源行；重复表头可按真实列过滤。
- CSV/TXT 自动识别 UTF-8、UTF-16 和 GBK，CSV 导出增加 UTF-8 BOM；文件导入新增 `.xlsm`。
- 配置 JSON 改为原子写入；配置包改为受大小限制的单文件 ZIP 读取，不再解压任意归档内容或残留临时目录。
- Core 回归测试扩展到 `16/16`。

### 关于窗口布局修复

- 修复标题下方版本文字因构造期 `AutoSize` 高度尚未稳定而与标题重叠、只露出少量像素的问题。
- “检查更新”按钮与状态文字改为同行垂直居中，并缩短空闲状态下的窗口高度。
- 关于窗口改用 DPI 缩放和固定文本行高，避免不同字体缩放下再次发生遮挡。
- 关于窗口改为非模态单实例窗口，打开后仍可继续操作主界面，重复点击“关于”只会激活已有窗口。

### TableCleaner.Core 物理迁移

- 将领域模型和纯处理服务从 WinForms 项目物理迁移到独立的 `src/TableCleaner.Core`。
- 按 `Domain`、`Parse`、`ColumnMapping`、`GroupMerge`、`Replacement`、`Filtering`、`ExportNormalization`、`Validation` 分目录维护。
- WinForms 项目只保留剪贴板、文件、Excel、配置、界面和更新等适配层，不再通过 linked compile 引入旧目录源码。
- CSV 与 XLSX 导出统一经过 Core 的导出规范化和结构校验。
